    ...
    <div>WELCOME <%=user.getName()%>| <a href="/myItems">Իմ Հայտարարությունները</a>|<a href="/addItem">Ավելացնել</a>|
        ...
        <a href="/singleItem?id=<%=item.getId()%>">
            <div style="width: 105px;float: left">
                ...
                <div>
                    <%if (item.getPictureList()!=null && !item.getPictureList().isEmpty() &&item.getPictureList().get(0).getPicUrl() != null && !item.getPictureList().get(0).getPicUrl().equals(" ") && !item.getPictureList().isEmpty()) {%>
                    <img src="/getImage?pic_url=<%=item.getPictureList().get(0).getPicUrl()%>" width="100"/>
